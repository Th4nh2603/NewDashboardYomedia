...
/** Build Demo wizard uploads always land on demo SFTP under `/script/demo/`. */
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const email = normalizeTextLower(req.query.email);
    const special = normalizeTextLower(req.query.special);
    ...
    if (special === "manage-demo-uploads") {
      ...
      const scopeRaw = normalizeTextLower(req.query.scope);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!action) {
      throw new HttpError(400, "Missing activity action", {
        code: "BAD_REQUEST",
      });
    }
    if (!area) {
