  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.imageDataUrl) {
        setGeneratedImage(response.imageDataUrl);
      } else {
      ...
      if (errMessage.includes("GEMINI_API_KEY is missing")) {
        setHasApiKey(false);
        notify('Server Gemini API key is not configured.', 'error', 'Key Error');
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-160px)] text-center px-6">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-[#e0e0e0] mb-4">Server Key Required</h2>
          <p className="text-[#a3a3a3] mb-8 leading-relaxed">
            Gemini image generation requires GEMINI_API_KEY to be configured on the server.
          </p>
          <Button
