...
const Cinema = () => {
  ...
  const handleSelectKey = async () => {
    setHasKey(true);
    notify('Server Gemini key will be used for video generation.', 'info', 'Setup Complete');
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      setStatusMessage('Rendering video on the server...');
      ...
      if (response.videoDataUrl) {
        setVideoUrl(response.videoDataUrl);
    ...
    return (
      <div className="max-w-2xl mx-auto mt-20 text-center space-y-8 p-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl transition-colors duration-300">
        ...
        <div className="space-y-4">
          ...
          <p className="text-slate-500 dark:text-slate-400">Veo-3.1 requires GEMINI_API_KEY to be configured on the server.</p>
