/** File `{value}.js` — `value` từ apps/server/src/data/creative-demos.json */
export {
...
} from "./display-balloon-expandable-itvc";

export const BANNER_FORMAT_REGISTRY = {
  "display-masthead": {
    format: () =>
      import("./display-masthead").then((m) => ({
  ...
  "display-balloon-expandable-itvc": {
    format: () =>
      import("./display-balloon-expandable-itvc").then((m) => ({
